...
Smallbutton = Core.class(Sprite)
local font = TTFont.new("font/kaiu.ttf", 15,true)
...
function Smallbutton:init(upStateImgFile, downStateImgFile, value)
...
function Smallbutton:onMouseDown(event)
...
function Smallbutton:onMouseMove(event)
	if self.focus then
...
function Smallbutton:onMouseUp(event)
	if self.focus then
		self.focus = false
		self:updateVisualState(false)
...
function Smallbutton:onTouchesBegin(event)
	if self.focus then
		event:stopPropagation()
	end
end
...
-- if button is on focus, stop propagation of touch events
function Smallbutton:onTouchesMove(event)
	if self.focus then
		event:stopPropagation()
	end
end
...
-- if button is on focus, stop propagation of touch events
function Smallbutton:onTouchesEnd(event)
	if self.focus then
		event:stopPropagation()
	end
end
...
function Smallbutton:onTouchesCancel(event)
	if self.focus then
		self.focus = false;
		self:updateVisualState(false)
		event:stopPropagation()
...
function Smallbutton:updateVisualState(state)
	if state then
