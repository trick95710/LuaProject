...
AceSlideCombinePopup = Core.class(Sprite)

function AceSlideCombinePopup:init(config)
...
function AceSlideCombinePopup:findClosest(current)
...
function AceSlideCombinePopup:onMouseDown(event)
...
function AceSlideCombinePopup:onMouseMove(event)
...
function AceSlideCombinePopup:onMouseUp(event)
	if self.isFocus then
		if(self.conf.orientation == "horizontal") then
...
function AceSlideCombinePopup:add(elem, selected)
...
function AceSlideCombinePopup:addButton(image, image_pushed, callback, selected)
...
function AceSlideCombinePopup:applyToAll(callback)
	for i = 1, self.total do
		callback(self:getChildAt(i))
	end
end

function AceSlideCombinePopup:show()
	local last = 0
...
function AceSlideCombinePopup:gotoItem(ind)
...
function AceSlideCombinePopup:jumptoItem(ind)
	if(self.conf.orientation == "horizontal") then
...
function AceSlideCombinePopup:nextItem()
...
function AceSlideCombinePopup:prevItem()
	if self.cur ~= 1 then
		self:gotoItem(self.cur - 1)
	else
		self:gotoItem(1)
...
function AceSlideCombinePopup:firstItem()
	self:gotoItem(1)
end

function AceSlideCombinePopup:lastItem()
	self:gotoItem(self.total)
end